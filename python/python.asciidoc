...
or `compare(1,2)` will give the intended behaviour.

The fromer version should be used if the number and/or names of the arguments is
...
principle only a wrapper funtion which creates a `Popen` object and waits until
...
constructure changed, the `call` function does not have to be changed at all. Thus
